...
namespace Generated.AI.Planner.StateRepresentation.RobberPlan
{
    ...
    public static class TraitArrayIndex<T> where T : struct, ITrait
    {
        ...
        static TraitArrayIndex()
        {
            ...
            if (typeIndex == TypeManager.GetTypeIndex<cop>())
                Index = 0;
            else if (typeIndex == TypeManager.GetTypeIndex<Location>())
                Index = 1;
            else if (typeIndex == TypeManager.GetTypeIndex<Robber>())
                Index = 2;
    ...
    public struct TraitBasedObject : ITraitBasedObject, IEquatable<TraitBasedObject>
    {
        ...
        public bool MatchesTraitFilter(NativeArray<ComponentType> componentTypes)
        {
            for (int i = 0; i < componentTypes.Length; i++)
            {
                ...
                else if (t.TypeIndex == s_copTypeIndex)
                {
                    if (t.AccessModeType == ComponentType.AccessMode.Exclude ^ copIndex == Unset)
                        return false;
                }
                ...
                else if (t.TypeIndex == s_RobberTypeIndex)
                {
                    if (t.AccessModeType == ComponentType.AccessMode.Exclude ^ RobberIndex == Unset)
                        return false;
                }
                else if (t.TypeIndex == s_TreasureTypeIndex)
                {
        ...
        public bool Equals(TraitBasedObject other)
        {
                ...
                return copIndex == other.copIndex && LocationIndex == other.LocationIndex && RobberIndex == other.RobberIndex && TreasureIndex == other.TreasureIndex && PlanningAgentIndex == other.PlanningAgentIndex;
        }
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                    ...
                    var hashCode = copIndex.GetHashCode();
    ...
    public struct StateData : ITraitBasedStateData<TraitBasedObject, StateData>
    {
        ...
        public StateData Copy(int jobIndex, EntityCommandBuffer.ParallelWriter entityCommandBuffer)
        {
            ...
            var cops = entityCommandBuffer.SetBuffer<cop>(jobIndex, stateEntity);
            cops.CopyFrom(copBuffer.AsNativeArray());
            var Locations = entityCommandBuffer.SetBuffer<Location>(jobIndex, stateEntity);
            Locations.CopyFrom(LocationBuffer.AsNativeArray());
            var Robbers = entityCommandBuffer.SetBuffer<Robber>(jobIndex, stateEntity);
        ...
        public void AddObject(NativeArray<ComponentType> types, out TraitBasedObject traitBasedObject, TraitBasedObjectId objectId, FixedString64 name = default)
        {
            ...
            {
                var t = types[i];
                if (t.TypeIndex == s_copTypeIndex)
                {
                    copBuffer.Add(default);
                    traitBasedObject.copIndex = (byte) (copBuffer.Length - 1);
                }
                else if (t.TypeIndex == s_LocationTypeIndex)
                {
                ...
                {
                    RobberBuffer.Add(default);
                    traitBasedObject.RobberIndex = (byte) (RobberBuffer.Length - 1);
                }
                else if (t.TypeIndex == s_TreasureTypeIndex)
                {
            ...
            ref TraitBasedObject traitBasedObject)
        {
            unsafe
            {
                foreach (var type in sourceTraitTypes)
                {
                    ...
                    if (type == typeof(Generated.Semantic.Traits.RobberData))
                    {
                        var traitData = sourceEntityManager.GetComponentData<Generated.Semantic.Traits.RobberData>(sourceEntity);
                        var plannerTraitData = new Robber();
                        UnsafeUtility.CopyStructureToPtr(ref traitData, UnsafeUtility.AddressOf(ref plannerTraitData));
                        SetTraitOnObject(plannerTraitData, ref traitBasedObject);
                    }
                }
        ...
        {
            if (trait is cop copTrait)
                SetTraitOnObject(copTrait, ref traitBasedObject);
            else if (trait is Location LocationTrait)
                SetTraitOnObject(LocationTrait, ref traitBasedObject);
            else if (trait is Robber RobberTrait)
        ...
        {
            if (trait is cop copTrait)
                SetTraitOnObjectAtIndex(copTrait, traitBasedObjectIndex);
            else if (trait is Location LocationTrait)
        ...
        DynamicBuffer<T> GetBuffer<T>() where T : struct, ITrait
        {
            ...
            {
                case 0:
                    return copBuffer.Reinterpret<T>();
                case 1:
                    return LocationBuffer.Reinterpret<T>();
                case 2:
                    return RobberBuffer.Reinterpret<T>();
                case 3:
        ...
        bool ObjectsMatchAttributes(TraitBasedObject traitBasedObjectLHS, TraitBasedObject traitBasedObjectRHS, StateData rhsState)
        {
            ...
            if (traitBasedObjectLHS.RobberIndex != TraitBasedObject.Unset
                && !RobberTraitAttributesEqual(RobberBuffer[traitBasedObjectLHS.RobberIndex], rhsState.RobberBuffer[traitBasedObjectRHS.RobberIndex]))
                return false;
        ...
        bool RobberTraitAttributesEqual(Robber one, Robber two)
        {
            return
                    one.Ready2steal == two.Ready2steal && 
        ...
        public override int GetHashCode()
        {
            ...
            bufferLength = RobberBuffer.Length;
            for (int i = 0; i < bufferLength; i++)
            {
                var element = RobberBuffer[i];
                var value = 397
                    ^ element.Ready2steal.GetHashCode()
        ...
        public override string ToString()
        {
            ...
            for (var traitBasedObjectIndex = 0; traitBasedObjectIndex < numObjects; traitBasedObjectIndex++)
            {
                ...
                if (traitIndex != TraitBasedObject.Unset)
                    sb.AppendLine(copBuffer[traitIndex].ToString());
                ...
                traitIndex = traitBasedObject[i++];
                if (traitIndex != TraitBasedObject.Unset)
                    sb.AppendLine(RobberBuffer[traitIndex].ToString());

                traitIndex = traitBasedObject[i++];
                if (traitIndex != TraitBasedObject.Unset)
                    sb.AppendLine(TreasureBuffer[traitIndex].ToString());
